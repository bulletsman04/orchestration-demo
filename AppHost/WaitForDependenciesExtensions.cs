...
public static class WaitForDependenciesExtensions
{
    ...
    static IDistributedApplicationBuilder AddWaitForDependencies(this IDistributedApplicationBuilder builder)
    {
    ...
    class WaitOnAnnotation(IResource resource) : IResourceAnnotation
    {
    ...
    class WaitForDependenciesRunningHook(
        ...
        IAsyncDisposable
    {
        ...
        public Task BeforeStartAsync(DistributedApplicationModel appModel, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var r in appModel.Resources)
            {
                ...
                r.Annotations.Add(new EnvironmentCallbackAnnotation(async context =>
                {
                    ...
                    await resourceNotificationService.PublishUpdateAsync(r, s => s with { State = new("Waiting", KnownResourceStateStyles.Info) });
            ...
            _ = Task.Run(async () =>
                {
                    ...
                    {
                        return snapshot.State == "FailedToStart" ||
                    ...
                    {
                        if (waitingResources.TryGetValue(resourceEvent.Resource,
                                out ConcurrentDictionary<WaitOnAnnotation, TaskCompletionSource>? pendingAnnotations))
                        {
        ...
        async Task DoTheHealthCheck(ResourceEvent resourceEvent, TaskCompletionSource tcs)
        {
            ...
            if (healthCheckAnnotation?.HealthCheckFactory is { } factory)
            {
                ...
                try
                {
                    ...
                    {
                        var context = new HealthCheckContext { Registration = new HealthCheckRegistration("", check, HealthStatus.Unhealthy, []) };
